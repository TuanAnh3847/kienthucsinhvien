...
(async () => {
    ...
    await test('Practice: finance quick quiz, exam, timer, mistakes, and restart', async () => {
        ...
        try {
            ...
            const examSize = await page.evaluate(()=>deduplicateQuestions(QUESTION_BANK.filter(q=>q.sourceExam===1)).length);
            assert.equal(await page.evaluate(()=>currentSession.questions.length),examSize);
            assert(examSize >= 40, 'reviewed exam retains a substantial question set');
            ...
            assert((await page.locator('#result-box').innerText()).includes('1/'+examSize));
            assert.equal(await page.evaluate(()=>getAttempts().length),1);
            assert.equal(await page.evaluate(()=>getWrongIds().length),examSize-1);
            ...
            assert.equal(await page.evaluate(()=>currentSession.questions.length),examSize-1);
