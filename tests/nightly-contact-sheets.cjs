...
(async()=>{
 for(const route of JSON.parse(fs.readFileSync('firebase.json')).hosting.rewrites) {
  ...
  for(let i=0;i<files.length;i++) {
   tiles.push({input:await sharp(path.join(dir,files[i])).resize(480,333,{fit:'contain',background:'#f1f5f9'}).toBuffer(),left:(i%3)*480,top:Math.floor(i/3)*361+28});
 ...
 for(const width of [1440,1366,390,375]) for(const state of ['top','sticky','choices']) {
  const files=fs.readdirSync(dir).filter(f=>f.endsWith(`-${width}-${state}.png`));
  for(let offset=0;offset<files.length;offset+=4) {
   const batch=files.slice(offset,offset+4),w=width>1000?720:width,h=width>1000?500:844;
   const tiles=[];
   for(let i=0;i<batch.length;i++) {
    tiles.push({input:await sharp(path.join(dir,batch[i])).resize(w,h,{fit:'contain',background:'#f1f5f9'}).toBuffer(),left:(i%2)*w,top:Math.floor(i/2)*(h+28)+28});
