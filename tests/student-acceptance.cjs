...
(async()=>{const b=await chromium.launch({channel:'msedge',headless:true});try{
 ...
 for(const subject of ['NLKT','TCCN']){
  ...
  await p.locator('#question-list').scrollIntoViewIfNeeded();await p.waitForTimeout(600);await p.screenshot({path:path.join(out,`final-${subject}-${width}-practice.png`)});
